...
public:
	...
	*/
	inline bool IsZombie() {
		if (this->GetClientClass()->m_ClassID == Jockey
			|| this->GetClientClass()->m_ClassID == Spitter
			|| this->GetClientClass()->m_ClassID == Charger
			|| this->GetClientClass()->m_ClassID == Hunter
			|| this->GetClientClass()->m_ClassID == Smoker
			|| this->GetClientClass()->m_ClassID == Boomer
			|| this->GetClientClass()->m_ClassID == Tank)
			return true;

		return false;
	}
	M_NETVAR(m_lifeStateInfected, unsigned char, "CBasePlayer", "m_lifeState");

	const char* GetSequenceName(int n_Sequence) {
		typedef const char* (__thiscall* get_sequence_name)(C_BaseEntity*, int);
		static auto get_sequence_name_ = reinterpret_cast<get_sequence_name>(U::Offsets.m_dwSequenceName);
		return get_sequence_name_(this, n_Sequence);
	}
	inline bool IsAlive() {
		return (m_lifeStateInfected() == 0);
	}
	/*
	bool IsAlive()
	{
		if (constexpr int FSOLID_NOT_SOLID = 0x0004; GetSolidFlags().has_flag(FSOLID_NOT_SOLID))
			return false;

		const auto activity = GetCurrentActivity();
		return !((activity >= ActivityList::ACT_DIESIMPLE && activity <= ActivityList::ACT_DIERAGDOLL) || activity == ActivityList::ACT_DIE_BARNACLE_SWALLOW
				 || (activity >= ActivityList::ACT_DIE_FRONTSIDE && activity <= ActivityList::ACT_DIE_STANDING)
				 || (activity >= ActivityList::ACT_TERROR_DIE_FROM_STAND && activity <= ActivityList::ACT_TERROR_DIE_RIGHTWARD_FROM_SHOTGUN));
	}
	OFFSET(ActivityList, GetCurrentActivity, 0x1250)
	*/
	inline bool ValidEntity(int m_Sequence, int m_SolidFlags)
	{
		//auto m_Group = this->GetGroup();
		//auto m_Sequence = this->As<C_BaseAnimating*>()->m_nSequence();
		/* IsSolid(this->GetSolidType(), this->GetSolidFlags()) && !strstr(GetSequenceName(), "Death") && !this->IsBurning() */
		int classindex = GetClientClass()->m_ClassID;
		const char* seqname = GetSequenceName(m_Sequence);
		if (classindex == Tank) {
			if (!IsAlive() && !(GetHealth() <= GetMaxHealth()))
				return false;
		}
		if (classindex == Jockey
			|| classindex == Spitter
			|| classindex == Charger
			|| classindex == Hunter
			|| classindex == Smoker
		...
		if (classindex == Infected)
		{
			if (m_SolidFlags & 4) // infected, 4, others, 16.
				return false;
		}
		if (seqname[0] == 'D')
		{
