...
{
	/* I can hook Fucking WndProc Sigless.*/
	/* Why ayyware? Why do this with a sig?*/
	while (!Game)
	{
		Game = FindWindowW(L"Valve001", nullptr);
		Sleep(100);
	}
	m_ulOldWndProc = reinterpret_cast<WNDPROC>(SetWindowLongPtrW(Game, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(WndProc)));

	if (!m_ulOldWndProc)
		throw std::runtime_error("[InputSys] SetWindowLongPtr failed.");
}

void InputSys::Uninit()
{
	if (m_ulOldWndProc)
		SetWindowLongPtrW(Game, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(m_ulOldWndProc));
}
